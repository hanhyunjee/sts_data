			...
			<c:forEach var="i" begin="0" end="${fn:length(historyList)-1}"
				...
				<c:choose>
					<c:when test="${status.index > 0}">
						...
						<c:otherwise>
							<tr>
								<td><fmt:formatDate value="${historyList[i].purchase_date}"	pattern="E MMM dd HH:mm:ss z yyyy" /></td>
								<td>${historyList[i].id}</td>
								<td>${productName[i]}...</td>
								<td>${historyList[i].grandTotal }원</td>
								<td>
									<form action="<c:url value="/historyDetail/${historyList[i].id }/${historyList[i].purchase_date }"/>" method="post" accept-charset="utf-8">
